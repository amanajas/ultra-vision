...
public class RentStatusDAO extends DAO implements IRentStatusDAO {
	...
	public int insertRentStatus(int userId, int rentalId, boolean status) throws SQLException {
		String sql = "INSERT INTO rental_status(id, created, updated, status, user_id, rental_id) VALUES(null, #1, #2, #3, #4, #5);";
		int id = this.db.insert(sql, NumberUtils.getDate().getTime(), 
				NumberUtils.getDate().getTime(), status ? 1 : 0, userId, rentalId);
		return id;
	}
	...
	public boolean updateRentStatus(int rentalStatusId, int userId, int rentalId, boolean status) throws SQLException {
		...
		boolean updated = this.db.update(sql, NumberUtils.getDate().getTime(), status ? 1 : 0, userId, rentalId,
				rentalStatusId);
		return updated;
	}
		...
		if (result != null) {
			result.forEach((map) -> {
				try {
					...
					/**
					 * status_id, user_id, rental_id, created, updated, status
					 */

					rents.add(new RentalStatus((int) map.get("status_id"),
							(User) this.user.getByID((int) map.get("user_id")),
							(Rental) this.rental.getByID((int) map.get("rental_id")), (int) map.get("status") == 1,
							NumberUtils.getDate(map.get("created")), NumberUtils.getDate(map.get("updated"))));
	...
	public List<RentalStatus> get(int userId) throws SQLException {
		String sql = "SELECT rs.id AS status_id, u.id AS user_id, r.id AS rental_id, r.title AS title, "
				+ "rs.created AS created, rs.updated AS updated, rs.status AS status " + "FROM rental_status AS rs "
				+ "JOIN rentals AS r ON rs.rental_id=r.id " + "JOIN users AS u ON u.id=rs.user_id "
				+ "WHERE rs.user_id=#1;";
	...
	public Object getByID(int id) {
		String sql = "SELECT rs.id AS status_id, u.id AS user_id, r.id AS rental_id, r.title AS title, "
				+ "rs.created AS created, rs.updated AS updated, rs.status AS status " + "FROM rental_status AS rs "
				+ "JOIN rentals AS r ON rs.rental_id=r.id " + "JOIN users AS u ON u.id=rs.user_id " + "WHERE rs.id=#1;";
	...
	public boolean hasBooking(int userId, int rentalId) throws SQLException {
		String sql = "SELECT count(*) as count " + "FROM rental_status AS rs "
				+ "WHERE rs.user_id=#1 AND rs.rental_id=#2;";
